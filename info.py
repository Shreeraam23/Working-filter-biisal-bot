...
BOT_TOKEN = environ.get('BOT_TOKEN', "7212050974:AAGcuwdt0Gpz56s_jFj6SoE4NMz2T_gdGHE") #if you are going to bot token here make sure your repo is private..
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6994050538').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002173398000').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '6994050538').split()]
auth_channel = environ.get('AUTH_CHANNEL')
auth_grp = environ.get('AUTH_GROUP', '')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002176628109')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002221103879')
REQST_CHANNEL = int(reqst_channel) if reqst_channel and id_pattern.search(reqst_channel) else None
SUPPORT_CHAT_ID = int(support_chat_id) if support_chat_id and id_pattern.search(support_chat_id) else None
NO_RESULTS_MSG = bool(environ.get("NO_RESULTS_MSG", False))

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Holaram:Holaram@cluster0.3bjr1mo.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/+Dk1S7y3tPadiYWVl')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/moneyearningsh')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002219999399')).split()]
